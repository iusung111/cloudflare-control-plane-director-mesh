...
import { createObservabilityRoute } from "../api/observability.route";
...
import { createRequestsRoute } from "../api/requests.route";
...
export function createApiRouter(services: AppServices, env?: WorkerEnv): Hono<{ Bindings: WorkerEnv }> {
  ...
  app.route("/observability", createObservabilityRoute(services));
  app.route("/queue", createQueueRoute(services));
  app.route("/quality", createQualityRoute(services));
  app.route("/requests", createRequestsRoute(services));
  ...
  app.route("/approvals", createApprovalsRoute(services, env));
  app.route("/alerts", createAlertsRoute(services, env));
  return app;
}
